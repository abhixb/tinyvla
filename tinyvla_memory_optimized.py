#!/usr/bin/env python3
"""
TinyVLA with Memory Optimizations

This is a modified version of tinyvla.py with added support for:
- 4-bit/8-bit model quantization
- 8-bit optimizers
- Better memory tracking

Changes from original:
1. Added use_quantization parameter to ModelArguments
2. Modified load_model() to support quantization
3. Added prepare_model_for_kbit_training for quantized + LoRA
4. Added memory logging utilities

Usage:
    # With 4-bit quantization (recommended for 12GB GPUs)
    python tinyvla_memory_optimized.py --config config_12gb.yaml

    # With 8-bit quantization
    python tinyvla_memory_optimized.py --config config.yaml --use_quantization 8bit
...
def load_model(model_id: str, use_flash_attention: bool = False, use_quantization: Optional[str] = None):
    """
    Load Qwen2.5-VL for training with optional quantization.

    Args:
        model_id: HuggingFace model ID
        use_flash_attention: Whether to use flash attention
        use_quantization: Quantization type: None, "4bit", or "8bit"
    ...
    if use_quantization == "4bit":
        print("  → Using 4-bit quantization (NF4 + double quantization)")
    ...
    # Flash attention (not compatible with quantization)
    if use_flash_attention and use_quantization is None:
        try:
        ...
        if use_flash_attention and use_quantization:
            print("  → Flash Attention disabled (incompatible with quantization)")
        model = Qwen2_5_VLForConditionalGeneration.from_pretrained(model_id, **kwargs)
...
class ModelArguments:
    ...
    use_quantization: Optional[str] = field(
        default=None,
        metadata={"help": "Model quantization: None, '4bit', or '8bit'"}
    )
...
def train():
    """Train VLA using TRL's SFTTrainer with memory optimizations."""
    ...
    print("\n" + "="*60)
    print("MEMORY-OPTIMIZED VLA TRAINING")
    print("="*60)
    print(f"Model: {model_args.model_id}")
    print(f"Quantization: {model_args.use_quantization or 'None'}")
    print(f"LoRA: {lora_args.use_lora}")
    ...
    if lora_args.use_lora and model_args.use_quantization:
        print("\nPreparing model for k-bit training...")
        from peft import prepare_model_for_kbit_training
        model = prepare_model_for_kbit_training(model)
        print("✓ Model prepared for k-bit training")
        print_memory_stats("after k-bit preparation")
    ...
    print(f"\n✓ Model saved to {training_args.output_dir}/final")
    ...
    if eval_args.eval_after_train:
        ...
        print(f"\nRunning eval-after-train: suite={suite}, task_id={eval_task_id}, episodes={eval_args.eval_n_episodes}")
        out = evaluate(
            final_dir,
            suite=suite,
